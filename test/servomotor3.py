...
'''
'''
for arto in [2, 3, 4, 5, 6, 7, 8, 9]:
	pwm.set_pwm(arto, 0, map(0))
	time.sleep(0.3) 
	pwm.set_pwm(arto, 0, map(-20))
	time.sleep(0.3) 
	pwm.set_pwm(arto, 0, map(20))
	time.sleep(0.3) 
	pwm.set_pwm(arto, 0, map(-40))
	time.sleep(0.3) 
	pwm.set_pwm(arto, 0, map(40))
	time.sleep(0.3) 
	pwm.set_pwm(arto, 0, map(0))
	time.sleep(0.6) 
'''
pwm.set_pwm(3, 0, map(50))
pwm.set_pwm(5, 0, map(-50))
pwm.set_pwm(7, 0, map(-50))
pwm.set_pwm(9, 0, map(50))
#scanning(0.3)
time.sleep(1)
pwm.set_pwm(3, 0, map(-40))
pwm.set_pwm(5, 0, map(40))
pwm.set_pwm(7, 0, map(40))
pwm.set_pwm(9, 0, map(-40))
time.sleep(0.2)
pwm.set_pwm(2, 0, map(-20))
pwm.set_pwm(4, 0, map(20))
pwm.set_pwm(6, 0, map(20))
pwm.set_pwm(8, 0, map(-20))
time.sleep(0.2)
pwm.set_pwm(3, 0, map(50))
pwm.set_pwm(5, 0, map(-50))
pwm.set_pwm(7, 0, map(-50))
